!(function () {
  ...
  function d3_documentElement(node) {
    return (
      node && (node.ownerDocument || node.document || node).documentElement
    );
  }
  ...
  function d3_selection_creator(name) {
    function create() {
      ...
      return namespace === d3_nsXhtml &&
        document.documentElement.namespaceURI === d3_nsXhtml
    ...
    function bind(group, groupData) {
      ...
      if (key) {
        ...
        for (i = -1; ++i < n; ) {
          if ((node = group[i])) {
            if (
              nodeByKeyValue.has((keyValue = key.call(node, node.__data__, i)))
            ) {
              exitNodes[i] = node;
            } else {
  ...
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    return (
      (polarAngle < -ε || (polarAngle < ε && d3_geo_areaRingSum < -ε)) ^
      (winding & 1)
    );
  }
